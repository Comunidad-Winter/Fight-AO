...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    PuedeFundarUnClan = False
        If TieneObjetos(772, 3, UserIndex) = False Then
            refError = "Necesitas 3 trofeos de oro para poder crear un Clan." 'En esta línea pueden modifiar lo que está entre comillas por el mensaje que se retorna en caso de que el usuario no tenga el Item o la cantidad necesaria.
            Exit Function
        End If
    ...
    If UserList(UserIndex).Stats.ELV < 54 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 90 Then
        refError = "Para fundar un clan debes ser nivel 54 y tener 90 en liderazgo."
        Exit Function
    End If
